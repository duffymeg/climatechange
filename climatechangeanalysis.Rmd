...
```{r, load F2017 data}
ccdata <- read.csv("ccsurvey_F17.csv", na.strings = "NA")
colnames(ccdata)
...
### dot plot that shows paired responses
```{r, dot plot of combined cchappening data}
cchappeningwidecombined <- dplyr::select(cctookbothcombined, CC_Happening_pre, CC_Happening_post)

cchappeningwidecombined <- cchappeningwidecombined %>%
  subset(CC_Happening_pre != "" & CC_Happening_post != "" ) 

str(cchappeningwidecombined)

# Was having a problem with one of the NAs not going away. Troubleshooting here to find it:
findingnas <- is.na(cchappeningwidecombined$CC_Happening_pre)
findingnas <- data.frame(findingnas)

# Now removing that row manually. Not elegant, but effective
cchappeningwidecombined = cchappeningwidecombined[-343,]

cchappeningwidecombined$CC_Happening_pre <- plyr::mapvalues(cchappeningwidecombined$CC_Happening_pre, from = c("YesExtrSure",
                                              "YesVerySure", 
                                              "YesSomeSure",
                                              "YesNotSure",
                                              "DK",
...
cchappeningwidecombined$CC_Happening_post <- plyr::mapvalues(cchappeningwidecombined$CC_Happening_post, from = c("YesExtrSure",
                                              "YesVerySure", 
                                              "YesSomeSure",
                                              "YesNotSure",
                                              "DK",
...
cchappeningwidecombined$CC_Happening_pre <- factor(cchappeningwidecombined$CC_Happening_pre, 
                                 c("No, somewhat sure",
                                  "I don't know",
                                  "Yes, but not sure",
                                  "Yes, somewhat sure",
...
cchappeningwidecombined$CC_Happening_post <- factor(cchappeningwidecombined$CC_Happening_post, 
                                 c("No, somewhat sure",
                                  "I don't know",
                                  "Yes, but not sure",
                                  "Yes, somewhat sure",
...
ntoplot <- length(cchappeningwidecombined$CC_Happening_pre)

correlationplotcombined <- ggplot(cchappeningwidecombined, aes(x=CC_Happening_pre, y=CC_Happening_post)) +
  ...
  theme_bw() +
  annotate("text", x = 6, y = 1, label = paste("n = 371"))

correlationplotcombined

save_plot("correlationplotcombined.jpg", correlationplotcombined, base_width = 8, base_height = 6)
```

### Making a combined figure for the manuscript
```{r, combined figure showing student views on climate change this time using the happening dot plot instead of the line plot}
combinedcchappeningplotcombinedv2 <- plot_grid(cchappeninglikertplotcombined, correlationplotcombined, labels=c('A', 'B'), nrow=2, ncol=1)
combinedcchappeningplotcombinedv2

save_plot("combinedcchappeningplotcombinedv2.jpg", combinedcchappeningplotcombinedv2, base_width = 10, base_height = 8)
```
...
CCHarmPeopleDataBind$prepost <- factor(CCHarmPeopleDataBind$prepost, c("beginning","end"))
...
CCChangedMePlotcombined <- ggplot(CCChangedMePostcombined, aes(x=Answer)) +
  ...
  annotate("text",x=1,y=105,label="n = 383",size=3) +
  theme_bw() +
  ylab("Number of responses")
...
CCchangedmesummary <- CCChangedMePostcombined %>%
  filter(Answer != "") %>%
  group_by(Answer) %>%
  summarise(n=n()) %>%
  mutate(rel.freq = round(100 * n/sum(n), 0))
...
ntoplot <- CCChangedMePostcombined %>%
  filter(Answer != "") %>%
  summarise(n=n())
...
ccchangedmelikertplotcombined = plot(likert_ccchangedmecombined,legend.position="bottom",text.size=5) + 
 ...
 ggtitle("This course changed how I think about climate change (n = 383)")  +
    theme(plot.title = element_text(size = 16, face = "plain")) 
...
```{r, course changed views using just fall 2017 data}
...
CCchangedmesummary <- CCChangedMePost %>%
  filter(Answer != "") %>%
  group_by(Answer) %>%
  summarise(n=n()) %>%
  mutate(rel.freq = round(100 * n/sum(n), 0))
...
```{r, making Likert style plot on changing views data using F17 data}
...
CCchangemindsummary <- CCChangeMindDataBind %>%
  filter(Answer != "") %>%
  group_by(Answer) %>%
  summarise(n=n()) %>%
  mutate(rel.freq = round(100 * n/sum(n), 0))
...
NOTE: I'm not sure why we asked this on the pre- and post- surveys, but it's interesting that we did!
...
save_plot("combinedworryimportantplot.jpg", combinedworryimportantplot, base_width =12, base_height = 8)
```
...
```{r, trying a stacked bar chart for friends data}
...
###########################################################
# OLD ANALYSES, HANGING OUT DOWN HERE JUST IN CASE
...
### trying a heat map
```{r, heat map of individual views on whether climate change is happening change during semester}
library(lattice)
...
### Making a second figure for the manuscript
```{r, combined figure showing student views on climate change this time using the dot plot instead of the line plot}
combinedcchappeningplotv2 <- plot_grid(cchappeninglikertplot, correlationplot, labels=c('A', 'B'), nrow=2, ncol=1)
combinedcchappeningplot

save_plot("combinedcchappeningplotv2.jpg", combinedcchappeningplotv2, base_width = 10, base_height = 6)
```
...
```{r, trying a stacked bar chart for consensus cause data v2}
...
```{r, more plotting of whether students think they can easily change their views}
CCChangeMindPre <- cctookboth$CC_ChangeMind_pre
CCChangeMindPre <- as.data.frame(CCChangeMindPre)
names(CCChangeMindPre)[1] = c("Answer")
...
```{r, summarize data}
CHappeningPrew18 <- cctookbothw18$CCPre_Happening_pre
